...
Целью данного проекта является расчет некоторых параметров открытого планарного диэлектрического волновода. В ходе работы над проектом я планирую взять на себя задачи по программированию и формированию отчетов. Ожидаю, что в ходе командной работы над проектом мы научимся координировать свои усилия, разделять задачи, управлять временем и рисками. Из практических навыков планирую освоить работу в пакетами компьютерной алгебры и символьных вычислений, и навык чтения литературы по тематику физических исследований и математического моделирования.
...
\begin{enumerate}
    \item Из решений уравнений (1-4) с учетом граничных условий и условия для существования нетривиального решения вывести зависимость $\beta$ от z при фиксированном d.
...
\begin{enumerate}
    ...
    \item С нахождение зависимости $\beta$ от z.
...
Имеем уравнения (5-8) в каждом слое (f, s, c) и граничные условия (a-d). Подставим уравнения (5-8) в граничные условия (a-d). Для граничных условий (a-c) заметим, что они эквивалентны следующим уравнениям:
$$\begin{cases}
E_y^c = E_y^f\\
H_y^c = H_y^f
\end{cases}
...
\end{cases}
$$
\par Для выполнения граничных условий на бесконечности (d), необходимо чтобы при $x \to \pm \infty$ компоненты $H_z, E_z$ в слоях $s$ ($+\infty$), $c$ ($-\infty$) стремились к нулю. Это достигается путем обнуления константы при возрастающих коэффициентах.

После подстановки компонент (5-8) в граничные условия, получаем СЛАУ для мод TE, TM. Они выглядят следующим образом:
$$\begin{cases}
...
\subsection{Через условие $det(\mathcal{M}(\beta, d)) = 0$ ищем значения для $\beta$}

Имеем определители: $$D_{TE} = det(\mathcal{M}_{TE}), D_{TM} = det(\mathcal{M}_{TM})$$
Уравнения имеют нетривиальное решение при условии $D_{TE} * D_{TM} = 0$. Опишем в общем случае методику нахождения зависимости $\beta(z)$.
\par По условию $D_i, i \in \{TM, TE\}$ - зависит от двух переменных $d, \beta$, где $d$ - ширина слоя. Для каждого $d_i \in (d_1, d_2, \ldots, d_n)$ зафиксируем $D_i(\beta) = D_i(\beta, d_i)$. Далее, для нахождения $\beta$ решим задачу минимизации $$argmin_{\beta} |D_i(\beta)|^2$$

Модуль используется с целью того, чтобы найти значения $\beta_i$ для $D_i(\beta)$, такие, что $D_i(\beta_i) = 0$

Решая такую задачу для каждого $d_i$ получаем набор пар $$B = \left( \{d_1 \to \beta_1\}, \{d_2 \to \beta_2\}, \ldots, \{d_n \to \beta_n\}  \right )$$
